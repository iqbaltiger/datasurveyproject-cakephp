...
class UsersController extends AppController {
	...
	public function beforeFilter(){
		parent::beforeFilter();
		$this->Auth->allow('forgot_password','reset_password','getUserUrl');	
	}
	...
	public function getUserUrl($data){
		...
		$userinfo = $this->User->find('first',array('conditions'=>array('User.username'=>$data['User']['username'],'password'=>$password)));
		
		$redirect_url = $this->Auth->redirect();
		if(!empty($userinfo['Role']['login_url'])){
			$pathinfo = pathinfo($userinfo['Role']['login_url']);
			
			$redirect_url = array('controller' => $pathinfo['dirname'], 'action' => $pathinfo['basename'],'prefix'=>'admin', 'admin'=>true);
		}
		return $redirect_url;
	}
